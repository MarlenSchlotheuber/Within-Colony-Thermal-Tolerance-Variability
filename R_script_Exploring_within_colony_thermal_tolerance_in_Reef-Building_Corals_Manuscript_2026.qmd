...
# rename Condition of MY to make it consistent with ID
eds_df_MY <- eds_df_MY %>%
...
eds_all_sites <- bind_rows(
  eds_df_MY %>% mutate(Country = "MY"),
  eds_df_ID %>% mutate(Country = "ID")
)
...
cbass_dataset_MY$Location <- factor(cbass_dataset_MY$Location, 
                                   levels = c("top", "upper_middle", "lower_middle", "bottom"))
...
ED50_diff_barplot_ID <- ggplot(Final_ED50_ID, aes(x = ED50_diff, y = Location, fill = Genotype)) +
...
# ED50 Violin Plot
ED50_violin_plot_ID <- ggplot(Final_ED50_ID, aes(x = 1, y = ED50, fill = Genotype)) +
  geom_violin() +
...
Final_ED50_MY  <- merge(cbass_dataset_MYS, ed50_df_MY)
...
row2 <- ED50_violin_plot_MY + ED50_diff_barplot_MY + plot_layout(ncol = 2, widths = c(0.4, 0.8))
...
# Malaysia
# filter data only for one temp so ed50s are not doubled
Final_ED50_MY <- Final_ED50_MY %>% 
...
# normallity test
shapiro.test(Final_ED50_MY$ED50) # p-value: 0.1506, data normally distributed 
# plot normallity
...
model <- lmer(ED50 ~ Location + (1 | Genotype), data = Final_ED50_MY)
anova(model)
...
# using a linear mixed model to account for nested design
model <- lmer(ED50 ~ Location + (1 | Genotype), data = Final_ED50_East)
anova(model)
...
# p-value = not signficant use ANOVA

# using a linear mixed model to account for nested design
model <- lmer(ED50 ~ Location + (1 | Genotype), data = Final_ED50_West)
anova(model)
# p-value: not significant

# Supplementary Table 11
# Spearman to check for correlation between ed50 and depth
...
its2_data_long_filtered <- its2_data_long_filtered %>%
  mutate(collection_location = dplyr::recode(collection_location,
...
#plot
ITS2_plot_MYS <- ggplot(its2_profile_MY_long_filtered, aes(x = value, y = collection_location, fill = ITS2_profile)) +
  geom_bar(stat = "identity", position = "fill") +
  scale_x_continuous(labels = scales::percent_format(), expand = c(0, 0)) +
...
# only select needed columns
its2_MY <- its2_MY %>%
  select(Genotype, Orientation, Location, ITS2_profile, value)

# make the data frame wide just with ITS2 type profile data
ITS2_MY_wide <- its2_MY %>%
  pivot_wider(
    id_cols = c(Genotype, Orientation, Location),
...
# put datafram,es together
combined_MY <- ITS2_MY_wide %>%
  left_join(
    VBA_Bacteria_MY_wide %>%
      select(SampleID, Genotype, Orientation, Location,
...
# mopve metadata columns to the front
...
ITS2_permanova <- adonis2(
  dist_ITS2 ~ Genotype + Orientation + Location + Depth,
  data = metadata,
  permutations = 999,
  method = "bray",
  by = "margin"   
